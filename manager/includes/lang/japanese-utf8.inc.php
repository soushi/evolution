...
$_lang["captcha_words_default"] = 'pyonkichi,hiroshi,kyouko,ume,minami,yoshiko,goriraimo,goro,mogura,machida';
...
$_lang["export_site_message"] = '<p>サイト全体をHTMLファイルとしてエクスポート(一括書き出し)できます。書き出したファイルをまとめてサーバ上に転送すれば、通常のサイトと同様の運用が可能です。</p><ul><li>エクスポートファイルは静的なhtmlファイルなので、動的なスニペットは動作しません。</li><li>リソースにリダイレクトヘッダを送信するスニペットが含まれている場合、エクスポートが失敗することがあります。</li><li>リソースID単位でエクスポートを行ないます。たとえば、10個の公開リソースがある場合は10ページ生成します。そのうちのひとつがページングなどで数ページを管理するものであっても処理しません。</li></ul><p><a href="index.php?a=17">グローバル設定</a>で「フレンドリーURLの使用」を「はい」にしてください。サイトの規模によっては、エクスポートにしばらく時間を要する場合もあります。</p><p><em>ファイル名が同じ場合、新しいファイルは既存のファイルに上書きされます</em></p>';
...
$_lang["php_version_check"] = 'MODXはPHPバージョン4.4.xまたはそれ以上で動作します。インストールされているPHPをアップグレードしてください。';
$_lang["plugin"] = 'プラグイン';
...
$_lang["role_save_chunk"] = 'チャンクの更新';
$_lang["role_save_doc"] = 'リソースの更新';
$_lang["role_save_module"] = 'モジュールの更新';
$_lang["role_save_password"] = 'パスワードの更新';
$_lang["role_save_plugin"] = 'プラグインの更新';
$_lang["role_save_role"] = 'ロールの更新';
$_lang["role_save_snippet"] = 'スニペットの更新';
$_lang["role_save_template"] = 'テンプレートの更新';
$_lang["role_save_user"] = 'ユーザーの更新';
$_lang["role_save_web_user"] = 'ウェブユーザーの更新';
...
$_lang["tmplvar_change_template_msg"] = 'このテンプレートを変更すると、このページのテンプレート変数をリロードします。更新していない変更は失われます。';
