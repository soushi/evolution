...
$_lang["configcheck_what"] = 'この警告の詳細';
...
$_lang["create_resource_title"] = 'リソースを作成';
$_lang["create_weblink_here"] = 'ウェブリンクを作成';
$_lang["createdon"] = '作成日時';
$_lang["credits"] = '著作権表示/謝辞';
$_lang["credits_shouts_msg"] = '<p>MODX is managed and maintained at <a href="http://modx.com/about/" target="_blank">modx.com</a>.</p>';
...
$_lang["export_site_message"] = '<p>サイト全体をHTMLファイルとしてエクスポート(一括書き出し)できます。書き出したファイルをまとめてサーバ上に転送すれば、通常のサイトと同様の運用が可能です。また、この機能は副次効果として、公開設定になっている全リソースのページキャッシュを一括生成します。</p><ul><li>エクスポートファイルは静的なhtmlファイルなので、動的なスニペットは動作しません。</li><li>リソースにリダイレクトヘッダを送信するスニペットが含まれている場合、エクスポートが失敗することがあります。</li><li>リソースID単位でエクスポートを行ないます。たとえば、10個の公開リソースがある場合は10ページ生成します。そのうちのひとつがページングなどで数ページを管理するものであっても処理しません。</li></ul><p><a href="index.php?a=17">グローバル設定</a>で「フレンドリーURLの使用」を「はい」にしてください。サイトの規模によっては、エクスポートにしばらく時間を要する場合もあります。</p><p><em>ファイル名が同じ場合、新しいファイルは既存のファイルに上書きされます</em></p>';
...
$_lang["htmlsnippet_management_msg"] = '<h3 style="font-weight:bold;">チャンクの管理</h3>チャンクはテキストのパーツです。テンプレートやリソース(投稿画面)に<strong>{{チャンク名}}</strong>と記述すると、そのチャンクの内容を出力します。';
...
$_lang["language_message"] = '管理画面の使用言語を選択します。必ず「エンコード」の設定と合わせてください。必要な言語ファイルが一覧にない場合は別途入手するか作成する必要があります。';
...
$_lang["remember_last_tab_message"] = 'タブ構成のページ(投稿画面・エレメント編集画面・グローバル設定など)において、最後に開いたタブを記憶します。「Stay mode」を選択すると「編集を続ける」を実行した時と一部の管理画面でタブ選択を記憶します。';
...
$_lang["resource_parent_help"] = '上のコンテナアイコンをクリックすると、このリソースの親(コンテナ)を選択できる状態になります。アイコンが変化している状態で、親にしたいリソースをリソースツリー上でクリックしてください。もう一度クリックすると元に戻ります。';
...
$_lang["role_frames"] = '管理画面フレームを要求';
$_lang["role_help"] = 'ヘルプページを表示';
$_lang["role_home"] = '管理画面ホームを要求';
...
$_lang["search_results_returned_msg"] = '<strong>%s</strong>件のデータが見つかりました。多くの検索結果が表示される場合は、検索範囲を更に狭くしてください。左側二つのカラムでは、リソースツリーでリソースを探したり、リソースを表示したりすることができます。右側二つのカラムでは、 リソースの削除状態および公開状態が各々表示されます。';
...
$_lang["site_slogan_message"] = 'サイトの説明を入力してください。コンフィグ変数： [(site_slogan)]';
$_lang["site_slogan_title"] = 'サイトの説明';
...
$_lang["snippet_management_msg"] = '<h3 style="font-weight:bold;">スニペットの管理</h3>スニペットは主にリソースやテンプレートから呼び出すことを想定した簡易の機能拡張です。リソースまたはテンプレート内の任意の場所に <strong>[[スニペット名]]</strong> または <strong>[!スニペット名!]</strong> (キャッシュ無効)と記述するだけで手軽に呼び出せます。';
...
$_lang["tmplvar_change_template_msg"] = 'テンプレートを変更すると、編集途中のテンプレート変数の値は破棄されます。';
...
$_lang["tmplvars_management_msg"] = '<h3 style="font-weight:bold;">テンプレート変数の管理</h3>投稿画面上に入力フィールドを追加できます。テンプレート内の任意の場所に <strong>[*テンプレート変数名*]</strong> と記述して値を出力します。';
...
$_lang["value"] = '値';
$_lang["view"] = 'ここをクリック';
$_lang["view_child_resources_in_container"] = 'サブリソース';
...
$_lang["settings_friendlyurls_alert2"] = 'サブディレクトリにインストールしているため、.htaccessの内容を変更する必要があります。';
...
$_lang["document_data.static.php1"] = 'チェックしたリソースを移動';
...
$_lang["mutate_content.dynamic.php1"] = '公開開始日時・公開終了日時をリセットします';
$_lang["mutate_htmlsnippet.dynamic.php1"] = '公開開始日時・公開終了日時をリセットします';
$_lang["mutate_settings.dynamic.php1"] = '標準モード';
